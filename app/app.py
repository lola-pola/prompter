...
def app():
    ...
    with st.sidebar:
            usage = st.selectbox("usage", ["-- select ---", "act", "generate"])
            if usage == "act":
                data = csv_to_dict("prompts.csv")
                selection = st.selectbox("what prompt?",list(data.keys()))
            else:
                data = csv_to_dict("examples-data.csv")
                selection = st.selectbox("what prompt?",list(data.keys()))
                prompt_example = data[selection]
            context = st.text_area("few shot context", height=300, key="you are a bot very nice of a israeli startup",value=data[selection])
    ...
    if usage == "act":
        ...
        with st.spinner('waiting for response...'):
            st.write(chatterbot(prompt_example,context,max_tokens,temperature,engine))
    else:
        st.markdown('select what prompt usage')
